...
## 同步队列 ##
> AbstractQueuedSynchronizer是用同步队列来完成同步状态的管理。假设当前线程获取同步状态失败时，则把**当前线程**以及**等待信息**构造成为一个节点（Node)并将其加入同步队列中，同时会**阻塞当前线程**
当同步状态释放时，会把首节点中的线程唤醒，使其再次尝试获取同步状态。等待队列是"CLH"锁队列的变体。通常CLH锁是一个自旋锁。
